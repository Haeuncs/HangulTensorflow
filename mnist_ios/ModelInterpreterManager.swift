...
public enum ModelInterpreterConstants {
  ...
  public static let hangulModelFilename = "hangul_tensorflow"
...
public class ModelInterpreterManager {
  ...
  private var isModelHangul = true
  ...
  public func loadLocalModel(bundle: Bundle = .main, isHangul: Bool = true) -> Bool {
    guard let localModelOptions = localModelOptions else {
      print("Failed to load the local model because the options are nil.")
      return false
    }
    isModelHangul = isHangul
    
    return loadModel(options: localModelOptions, isHangul: isHangul, bundle: bundle)
  }
